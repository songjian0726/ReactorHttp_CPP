...
string HttpRequest::getHeader(const string key)
{
    ...
    if (item == m_reqHeaders.end()) return string();
    return item->second;
}
...
bool HttpRequest::parseRequestLine(Buffer* readBuf)
{
    ...
    if (lineSize > 0) {
        ...
        start = splitRequestLine(start, end, " ", methodFunc);//请求方式  最后一个参数是指针的地址 传出参数

        auto urlFunc = bind(&HttpRequest::setUrl, this, placeholders::_1);
        start = splitRequestLine(start, end, " ", urlFunc);//路径

        auto versionFunc = bind(&HttpRequest::setVersion, this, placeholders::_1);
        splitRequestLine(start, end, nullptr, versionFunc);//版本
...
bool HttpRequest::parseRequestHeader(Buffer* readBuf)
{
    ...
    if (end != nullptr) {
        ...
        int lineSize = end - start;
        //搜索:  分割键值对
        char* middle = static_cast<char*>(memmem(start, lineSize, ": ", 2));//浏览器中的标准http请求中 :后面有一个空格
        if (middle != nullptr) {
...
bool HttpRequest::parseHttpRequest(Buffer* readBuf, HttpResponse* response, Buffer* sendBuf, int socket)
{
    ...
    while (m_curState != PrecessState::ParseReqDone) {
        ...
        case PrecessState::ParseReqHeaders:
            ...
            break;
        case PrecessState::ParseReqBody://处理POST请求时需要考虑这部分 此处暂不处理
            break;
        default:
            break;
        }
        ...
        if (m_curState == PrecessState::ParseReqDone) { //解析完毕后 准备回复数据
            processHttpRequest(response);
            response->prepareMsg(sendBuf, socket);
        }
    }
    m_curState = PrecessState::ParseReqLine;//还原解析状态，便于处理后续请求
    return flag;
}
...
bool HttpRequest::processHttpRequest(HttpResponse* response)
{
    ...
    else
    {
        ...
        response->addHeader("Content-length", to_string(st.st_size)); //to_string直接将st结构体中的st_size(int)转换成字符串
        response->sendDataFunc = sendFile;
        //return true;
    }
    return false;
